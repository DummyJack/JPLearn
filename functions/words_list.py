...
class JapaneseTextInput(TextInput):
    def insert_text(self, substring, from_undo=False):
        ...
        japanese_chars = "".join(
            ...
            if re.match("^[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]$", char)
        )

        if japanese_chars:
            # Get current cursor position
            pos = self.cursor_index()
            # Update text property directly
            self.text = self.text[:pos] + japanese_chars + self.text[pos:]
            # Move cursor to end of inserted text
            self.cursor = (pos + len(japanese_chars), 0)
...
class EditWordPopup(Popup):
    def __init__(self, japanese, explanation, edit_callback, word_id, **kwargs):
        ...
        input_layout = BoxLayout(
            orientation="horizontal", spacing=dp(10), size_hint_y=None, height=dp(150)
        )
        ...
        button_layout = BoxLayout(
            orientation="horizontal", spacing=dp(20), size_hint_y=None, height=dp(40)
        )
...
class WordItem(BoxLayout):
    def __init__(
        ...
        # 按鈕列
        button_layout = BoxLayout(
            orientation="horizontal", size_hint_x=0.2, spacing=dp(5)
        )
        ...
        button_anchor = AnchorLayout(
            anchor_x="center", anchor_y="center", size_hint_x=None, width=dp(70)
        )
        button_inner_layout = BoxLayout(
    ...
    def edit_word(self, instance):
        popup = EditWordPopup(
            self.word, self.explanation, self.update_word, self.word_id
        )
        popup.open()
...
class WordsList(ScrollView):
    ...
    def show_delete_confirmation(self, word_item):
        content = BoxLayout(
            orientation="vertical", spacing=dp(40), padding=dp(20)
        )  # 增加間距
        content.add_widget(
            Label(text="是否刪除單字？", font_name="ChineseFont", font_size=dp(24))
        )

        buttons = BoxLayout(
            orientation="horizontal", spacing=dp(20), size_hint_y=None, height=dp(50)
        )  # 增加按鈕高度
        confirm_btn = Button(
            text="確定",
            font_name="ChineseFont",
        ...
        popup = Popup(
            content=content, size_hint=(0.6, 0.3), title="", separator_height=0
        )
...
class AddWordPopup(Popup):
    def __init__(self, add_word_callback, update_view_callback, **kwargs):
        super().__init__(**kwargs)
        self.title = ""
        self.separator_height = 0
...
class WordsListPopup(Popup):
    def __init__(self, **kwargs):
        ...
        word_header.add_widget(
            Label(text="單字", font_name="ChineseFont", font_size=dp(28))
        )
        header.add_widget(word_header)

        explanation_layout = BoxLayout(
            orientation="horizontal", size_hint_x=0.7, spacing=dp(20)
        )
        explanation_layout.add_widget(
            Label(
                text="解釋", font_name="ChineseFont", font_size=dp(28), size_hint_x=0.7
            )
        )
    ...
    def show_add_popup(self, instance):
        popup = AddWordPopup(
            add_word_callback=self.words_list.add_word,
    ...
    def update_pagination(self):
        self.page_label.text = (
            f"{self.words_list.current_page}/{self.words_list.total_pages}"
        )
    ...
    def show_search_popup(self, instance):
        ...
        search_button = Button(
            text="搜尋", font_name="ChineseFont", font_size=dp(18), size_hint_x=0.3
        )
